    ...
    **kwargs:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Elasticache 
        #self.elasticache_config_endpoint = "dask-cache.fvya82.cfg.use1.cache.amazonaws.com:11211"
        #self.nodes = elasticache_auto_discovery.discover(self.elasticache_config_endpoint)
        #self.nodes = map(lambda x: (x[1], int(x[2])), self.nodes)
        #self.memcache_client = HashClient(self.nodes)        
        
        #self.elasticache_redis_endpoint = "dask-storage-001.fvya82.0001.use1.cache.amazonaws.com"
        #self.elasticache_redis_endpoint = "dask-sl-storage.fvya82.ng.0001.use1.cache.amazonaws.com"
        #self.elasticache_redis_endpoint = "dasl-sl-storage.fvya82.clustercfg.use1.cache.amazonaws.com"
        #self.elasticache_redis_endpoint = "ec2-34-227-191-239.compute-1.amazonaws.com"
        #self.redis_client = redis.StrictRedis(host=self.elasticache_redis_endpoint, port = 6379, db = 0)        
    ...
    def _gather(self, futures, errors="raise", direct=None, local_worker=None):
        ...
        while True:
            ...
            if self._gather_future:  # attach onto another pending gather request
                ...
                self._gather_keys = set(keys)
                future = self._gather_from_redis()
                #future = self._gather_remote(direct, local_worker)
                #print("future: ", future)
                #print("future_: ", future_)
        ...
        # print("Result [in _gather]: ", result)
        raise gen.Return(result)

    @gen.coroutine
    def _gather_from_redis(self):
        """ Perform gather from Redis """
        ...
        print("[CLIENT] Retrieving values for keys: {}".format(keys))
        ...
        for key, value in values:
            if value is not None:
                ...
                print("[CLIENT] Obtained value {} for key {} from Redis.".format(value_deserialized, key))
    ...
    def _gather_remote(self, direct, local_worker):
        ...
        try:
            if direct or local_worker:  # gather directly from workers
                ...
                #print("response [from scheduler in _gather_remote]: ", response)
        finally:
            self._gather_semaphore.release()

        #print("Response [in _gather_remote]: ", response)
    ...
    def compute(
        ...
        **kwargs:
        ...
        start_graph2futures = datetime.datetime.utcnow()
        ...
        print("[ {} ]  self._graph_to_futures(...) took {} seconds to execute.".format(now, now - start_graph2futures))
        ...
        if singleton:
            now = datetime.datetime.utcnow()
            print("[ {} ]  Returning result from compute() in client.".format(now))
            return first(result)
        else:
            print("[ {} ]  Returning result from compute() in client.".format(now))
            now = datetime.datetime.utcnow()
