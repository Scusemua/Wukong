...
class Scheduler(ServerNode):
    ...
    * **coroutines:** ``[Futures]``:
    ...
    def __init__(
        ...
        big_task_threshold = 50,                     # The threshold, in bytes, above which an object should be broken up into chunks when stored.
    ...
    def update_graph(
        ...
        def DFS(current_task, current_path, current_path_size, visited, isNewPath = False, incrDepCounter = False):
            ...
            payload = self.construct_basic_task_payload(current_task.key, current_task, already_executed = already_executed)
            ...
            if self.use_bit_dep_checking == False:
                ...
                initial_payloads[self.dcp_redis][redis_dep_counter_key] = 0
        ...
        if fargate_launcher is not None:
        ...
        for task_key, path in tasks_to_path_starts.items():
            ...
            payload = {
                ...
                "is-leaf": leaf_tasks.get(task_key, False) and self.reuse_lambdas # If self.reuse_lambdas is False, then we don't care if this is a leaf task or not.
    ...
    def construct_basic_task_payload(self, task_key, ts, already_executed = False):
